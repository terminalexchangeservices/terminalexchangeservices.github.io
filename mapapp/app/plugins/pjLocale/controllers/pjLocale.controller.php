...
class pjLocale extends pjLocaleAppController
{
					...
					if ($handle)
					{
						...
						if (!empty($string))
						{
							...
							foreach ($arr as $v)
							{
								...
								if (!empty($v))
								{
									$v = preg_replace('/`model`\s*=\s*\'pjField\'\s*AND\s*`locale`\s*=\s*\'\d+\'\s*AND\s*`source`/', "`model` = 'pjField' AND `locale` = '$locale' AND `source`", $v);
									$v = preg_replace('/\'pjField\',\s*\'\d+\',\s*\'title\'/', "'pjField', '$locale', 'title'", $v);
									if (!mysql_query($v))
									{
										//return mysql_error();
										$err = 'PAL04';
	...
	public function pjActionIndex()
	{
		...
		{
			$q = pjObject::escapeString(trim($_GET['q']));
